...
def Prediction(pred_data, model, user_RT, item_RT, completion, \
    ...
    Prediction on targets = to be mentionned movies with rating of 1...
    ...
    with torch.no_grad():
        for batch_idx, (dataid, ConvID, qt_movies_mentionned, user_id, l_item_id, l_rating) in enumerate(pred_data):
          ...
          #  l_rating = ast.literal_eval(l_rating)
            ...
            ranks, avrg_rk, mrr, rr, re_1, re_10, re_50, ndcg = \
                                        Ranks(pred, targets, ranking_method, topx)
            
            
            # Add Ranks results to appropriate dict
            if qt_movies_mentionned in RR.keys():
                Avrg_Ranks[qt_movies_mentionned].append(avrg_rk)
                MRR[qt_movies_mentionned].append(mrr)
                RR[qt_movies_mentionned].append(rr)
                NDCG[qt_movies_mentionned].append(ndcg)
                RE_1[qt_movies_mentionned].append(re_1)
                RE_10[qt_movies_mentionned].append(re_10)
                RE_50[qt_movies_mentionned].append(re_50)
            else:
                Avrg_Ranks[qt_movies_mentionned] = [avrg_rk]
                MRR[qt_movies_mentionned] = [mrr]
                RR[qt_movies_mentionned] = [rr]
                NDCG[qt_movies_mentionned] = [ndcg]
                RE_1[qt_movies_mentionned] = [re_1]
                RE_10[qt_movies_mentionned] = [re_10]
                RE_50[qt_movies_mentionned] = [re_50]
            """ Done"""   
