...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 11 11:47:10 2019


Adapting file fast-bert.data_cls.py for recommendation because:
    1- Because size and spacity, labels are now [(item_id, rating)]
    2- Need a mask on unlabled items for loss evaluation (only on labeled data)

Changes are in:
    1- fct convert_examples_to_features: if labels are [(item_id, rating)], not treated here. 
...
# *** CHANGE ***
            # Check if for recommendation, do nothing (except convert to right type)
            # Processing done in modeling (definition of loss function)
            example_label_converted_to_list_tuples = literal_eval(example.label[0])
            if isinstance(example_label_converted_to_list_tuples[0], tuple):
                label_id = example_label_converted_to_list_tuples
