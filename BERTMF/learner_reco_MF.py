...
# *** CHANGE ***
from BERTMF.modeling_reco_MF import BertForMultiLabelSequenceClassification, XLNetForMultiLabelSequenceClassification, RobertaForMultiLabelSequenceClassification, DistilBertForMultiLabelSequenceClassification
import json
# *** CHANGE ***
...
# If in recommender case        
        if dataBunch.labels == ['ratings']:
            config = config_class.from_pretrained(pretrained_path, num_labels=1)
# If multi-label
        else:
...
# *** CHANGE ***       
    ...
    ### Evaluate the model. Only concerned with eval loss.    
    def validate(self):
        """
        ...
        Returns
        -------
        results: a dict of one key, 'eval_loss'      
        """
        ...
        for step, batch in enumerate(progress_bar(self.data.val_dl)):
            self.model.eval()
            # Put each element in the dict on the right device
            batch = tuple(t.to(self.device) for t in batch)
    ...
    def RankUser(self):
        ...
        print("''''' TRAGETS_IDX", target_idx, type(target_idx))
    ...
    def save_model(self): 
        ...
        path = Path(self.output_dir, 'Results', self.exp_id, self.saving_epoch)
